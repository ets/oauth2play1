...
import oauth2.common.message.types.ParameterStyle;
import oauth2.common.message.types.TokenType;
import oauth2.common.utils.OAuthUtils;
import oauth2.common.validators.OAuthValidator;
import oauth2.rs.BearerResourceServer;
import oauth2.rs.ResourceServer;
import oauth2.rs.extractor.TokenExtractor;
...
public class OAuthAccessResourceRequest {
    ...
    public OAuthAccessResourceRequest(Request request)
        throws OAuthSystemException, OAuthProblemException {
        this(request,new TokenType []{OAuth.DEFAULT_TOKEN_TYPE}, new ParameterStyle[] {OAuth.DEFAULT_PARAMETER_STYLE});
    }

    public OAuthAccessResourceRequest(Request request, ParameterStyle... parameterStyles)
    throws OAuthSystemException, OAuthProblemException {
    	this(request,new TokenType []{OAuth.DEFAULT_TOKEN_TYPE}, parameterStyles);
    }
    
    public OAuthAccessResourceRequest(Request request, TokenType... tokenTypes)
    throws OAuthSystemException, OAuthProblemException {
    	this(request,tokenTypes,  new ParameterStyle[] {OAuth.DEFAULT_PARAMETER_STYLE});
    }
    
    public OAuthAccessResourceRequest(Request request, TokenType[] tokenTypes ,ParameterStyle[] parameterStyles)
        throws OAuthSystemException, OAuthProblemException {
